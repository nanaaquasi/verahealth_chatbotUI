<template>
  <main>
    ...
    <section class="container chat-container">
      ...
      <section v-else aria-live="polite">
        <div v-for="message in messages" id="message" :key="message.responseId">
          ...
          <RichComponent
            v-for="(component, id) in message.queryResult.fulfillmentMessages"
            :key="id"
          >
          ...
          >
            <RichComponent
              v-for="(component, id) in message.queryResult.webhookPayload
    ...
    <div class="welcome_message" v-if="app && messages.length == 0">
      <p>Welcome, choose one of the suggestions below to start.</p>
    </div>
...
export default {
  ...
  created() {
    ...
    if (this.history() && sessionStorage.getItem("session") !== null) {
      ...
      // console.log({ sessionId: $route.params.id });
